...
from src.features.document_processing.loaders.component_classification import blocks_for_chunking, is_image_block
    ...
    def run(
        ...
        *,
        ...
        has_text_or_table_content = any(
            not is_image_block(block) and str(getattr(block, "text", "") or "").strip()
            for block in blocks
        )
        if resolved.chunking_strategy == "semantic-hierarchy":
            has_text_or_table_content = has_text_or_table_content or any(
                is_image_block(block)
                and bool(
                    str(
                        (getattr(block, "metadata", {}) or {}).get("caption")
                        or (getattr(block, "metadata", {}) or {}).get("image_caption")
                        or (getattr(block, "metadata", {}) or {}).get("description")
                        or ""
                    ).strip()
                )
                for block in blocks
