...
class RetrievalService:
    ...
    def _rerank_chunks(
        ...
        *,
        ...
        return result.chunks, result.applied, result.metrics if result.applied else None
    ...
    def _run_retrieval_strategy(
        ...
        *,
        ...
        vector_limit = int(body.candidate_k or pipeline_cfg.dense_limit or body.resolved_top_k)
        bm25_limit = int(body.candidate_k or pipeline_cfg.sparse_limit or body.resolved_top_k)
    ...
    def _rerank_candidates(
        ...
        *,
        ...
        if not result.applied:
            return candidates
        return [self._candidate_from_chunk(chunk) for chunk in result.chunks]
    ...
    async def retrieve(self, body: RetrieveRequest) -> RetrieveResponse:
        ...
        try:
            ...
            _logger.info(
                "Searching Weaviate collection=%s search_type=%s",
                collection_name,
                search_mode,
            )
            common = {
                "limit": body.resolved_top_k,
                "filters": where_filter,
                "return_metadata": MetadataQuery(distance=True, score=True, explain_score=body.explain),
            }
            if where_filter is None:
        ...
        try:
            ...
            if use_rerank:
                ...
                chunks, rerank_applied, rerank_metrics = self._rerank_chunks(
                    ...
                    chunks,
                    repo_settings=repo_settings,
                    top_k_before=body.rerank_top_k or rerank_cfg.top_k_before,
                    top_k_after=body.resolved_top_k,
                )
